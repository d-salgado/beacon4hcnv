...
async def snp_request_handler(db_pool, processed_request, request):
    ...
    variantId = str(variantId[0])
    LOG.debug(f"VariantId: {variantId}")

    resultsHandover = snp_resultsHandover(variantId) if variantId else ''
    ...
    beacon_response = { 'beaconId': '.'.join(reversed(request.host.split('.'))),
                        ...
                        'datasetAlleleResponses': filter_exists(processed_request.get("includeDatasetResponses", "NONE"), datasets),
                        'info': {'rs ID': variantId},
                        'resultsHandover': resultsHandover,
